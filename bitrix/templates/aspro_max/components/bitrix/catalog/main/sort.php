...
.cat__controls-switch-item.--active {
    background-color: #fff;
    pointer-events: none;
}
.cat__controls-switch-item {
    width: 36px;
    height: 25px;
    border-radius: 100px;
    display: -webkit-box;
...
<div class="filter-panel sort_header view_<?=$display?> <?=($bShowCompactHideLeft && $bHideLeftBlock ? 'show-compact' : '' );?> <?=(!$bShowSortInFilter ? 'show-normal-sort' : '' );?>">
	...
	<!--noindex-->
		...
		<div class="filter-panel__view controls-view pull-right">
			<div class="cat__controls-switch">	
			<?foreach($arDisplays as $displayType):?>
			<?if ($displayType === 'table') break;?>
				<?
				$current_url = '';
				...
				<?if($display == $displayType):?>
					...
					<span title="<?=\Bitrix\Main\Localization\Loc::getMessage("SECT_DISPLAY_".strtoupper($displayType))?>" class="controls-view__link controls-view__link--<?=$displayType?> cat__controls-switch-item --active"><?=CMax::showIconSvg("type", SITE_TEMPLATE_PATH.'/images/svg/catalog/'.$displayType.'type.svg', '', '', true, false);?></span>

				<?else:?>

					<a rel="nofollow" href="<?=$url;?>" data-url="<?=$url?>" title="<?=\Bitrix\Main\Localization\Loc::getMessage("SECT_DISPLAY_".strtoupper($displayType))?>" class="cat__controls-switch-item controls-view__link--<?=$displayType?> muted<?=($arParams['AJAX_CONTROLS'] == 'Y' ? ' js-load-link' : '');?>"><?=CMax::showIconSvg("type", SITE_TEMPLATE_PATH.'/images/svg/catalog/'.$displayType.'type.svg', '', '', true, false);?></a>
